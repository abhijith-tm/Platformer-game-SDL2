...
void loadGame(GameState *game)
{
  ...
  surface = IMG_Load("images/man_lta.png");
  if(surface == NULL)
  {
  ...
  surface = IMG_Load("images/man_ltb.png");
  if(surface == NULL)
  {
  ...
  //Load sounds
  game->bgMusic = Mix_LoadWAV("images/music.wav");
  if(game->bgMusic != NULL)
  {
    Mix_VolumeChunk(game->bgMusic, 32);
  }
  game->dieSound = Mix_LoadWAV("images/die.wav");
  game->jumpSound = Mix_LoadWAV("images/jump.wav");
  game->landSound = Mix_LoadWAV("images/land.wav");
...
void doRender(SDL_Renderer *renderer, GameState *game)
{
  ...
  else if(game->statusState == STATUS_STATE_GAME)
  {
    ...
    SDL_Rect rect = { game->scrollX+game->man.x, game->man.y, 48, 48 };
	    ...
	    // Use different flip logic depending on direction
    if(game->man.dx != 0) // If the man is moving, decide based on direction
    {
        // If moving left, flip the sprite horizontally
        SDL_RenderCopyEx(renderer, game->manFrames[game->man.animFrame],
                         NULL, &rect, 0, NULL, SDL_FLIP_HORIZONTAL);
    }
    else // If the man is standing still, just render the standing sprite
    {
        SDL_RenderCopy(renderer, game->manFrames[game->man.animFrame],
                       NULL, &rect);
    }
...
int main(int argc, char *argv[])
{
  ...
  window = SDL_CreateWindow("Game",                     // window title
                            ...
                            SDL_WINDOWPOS_UNDEFINED,           // initial y position                              // width, in pixels
                            640, 480,                              // height, in pixels
                            flags[mode]);                      // flags
